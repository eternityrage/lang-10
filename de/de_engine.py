...
VOICE_A = 'de-DE-AmalaNeural'
VOICE_B = 'de-DE-FlorianMultilingualNeural'

CATEGORIES = [
    'Bakery & Bretzel', 'Berlin Street Life', 'Directions & U-Bahn',
    'Cafe & Biergarten', 'Daily Greetings', 'Polite Requests'
]
...
def generate_ai_dialogue(category=None):
    ...
    prompt = (
        'Generate an ultra-simple, beginner-friendly 4-line German dialogue (Level 1 / A1 beginner) between Character A and Character B.\n'
        f'Category: {category}\n'
        'CRITICAL SIMPLICITY REQUIREMENTS:\n'
        '- Keep each sentence TINY, SIMPLE, and beginner-level (3 to 6 words max per sentence!).\n'
        '- Level: Absolute beginner everyday conversational German.\n'
        '- Anyone seeing the reel should be able to instantly memorize and repeat each line!\n'
        '- Only natural German and English translation (no transliteration needed).\n'
    ...
    payload = {
        'model': AI_MODEL,
        'messages': [
            {'role': 'system', 'content': 'You are an expert German teacher creating viral micro-dialogues for absolute beginners. Keep sentences short, clean, and memorable. Output ONLY valid JSON.'},
            ...
            if 'lines' in data:
                for idx, item in enumerate(data['lines'][:4]):
                    ...
                    if 'target_text' not in item:
                        for k in ['text', 'german', 'deutsch']:
                            if k in item: item['target_text'] = item[k]; break
                    item['japanese'] = item.get('target_text', '')
                    item['romaji'] = ''
                data['lines'] = data['lines'][:4]
            print('[Spracho German] Generated dialogue: ' + str(data.get('topic')))
            return data
    except Exception as e:
        print('[Spracho German] Fallback triggered: ' + str(e))

    return {
        'topic': category,
        'lines': [
            {'speaker': 'A', 'english': 'Hello! Is this bretzel fresh?', 'target_text': 'Hallo! Ist die Brezel frisch?', 'japanese': 'Hallo! Ist die Brezel frisch?', 'romaji': ''},
            {'speaker': 'B', 'english': 'Yes, fresh from the oven!', 'target_text': 'Ja, frisch aus dem Ofen!', 'japanese': 'Ja, frisch aus dem Ofen!', 'romaji': ''},
            {'speaker': 'A', 'english': 'One bretzel, please.', 'target_text': 'Eine Brezel, bitte.', 'japanese': 'Eine Brezel, bitte.', 'romaji': ''},
            {'speaker': 'B', 'english': 'Here you go, enjoy!', 'target_text': 'Bitte schön, guten Appetit!', 'japanese': 'Bitte schön, guten Appetit!', 'romaji': ''}
        ]
    }
...
def render_frame(speaker_speaking, is_talking, mouth_flap, line_data, font_en, font_jp, font_ro):
    ...
    raw_german = line_data.get('target_text', line_data.get('japanese', ''))
    wrapped_jp = wrap_text(raw_german, font_jp, max_w=920)
    ...
    gap_1 = 55  # Gap between English and German
...
async def generate_single_reel():
    ...
    reel_id = f'spracho_{timestamp}'

    font_en = ImageFont.truetype('C:/Windows/Fonts/segoeuib.ttf', 58)
    font_jp = ImageFont.truetype('C:/Windows/Fonts/segoeuib.ttf', 76)
    ...
    print('\n[Spracho German] Generated: ' + str(final_vid.name))
    return final_vid
