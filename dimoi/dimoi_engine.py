...
VOICE_A = 'fr-FR-DeniseNeural'
VOICE_B = 'fr-FR-HenriNeural'

CATEGORIES = [
    'Parisian Cafe & Bakery', 'Street Greetings', 'Metro & Travel',
    'Boutique Shopping', 'Romantic Flirting', 'Daily Life'
]
...
def generate_ai_dialogue(category=None):
    ...
    prompt = (
        'Generate an ultra-simple, beginner-friendly 4-line French dialogue (Level 1 / A1 beginner) between Character A and Character B.\n'
        f'Category: {category}\n'
        'CRITICAL SIMPLICITY REQUIREMENTS:\n'
        '- Keep each sentence TINY, SIMPLE, and beginner-level (3 to 6 words max per sentence!).\n'
        '- Level: Absolute beginner everyday conversational French.\n'
        '- Anyone seeing the reel should be able to instantly memorize and repeat each line!\n'
        '- Only natural French and English translation (no transliteration needed).\n'
    ...
    payload = {
        'model': AI_MODEL,
        'messages': [
            {'role': 'system', 'content': 'You are an expert French teacher creating viral micro-dialogues for absolute beginners. Keep sentences short, clean, and memorable. Output ONLY valid JSON.'},
            ...
            if 'lines' in data:
                for idx, item in enumerate(data['lines'][:4]):
                    ...
                    if 'target_text' not in item:
                        for k in ['text', 'french', 'francais']:
                            if k in item: item['target_text'] = item[k]; break
                    item['japanese'] = item.get('target_text', '')
                    item['romaji'] = ''
                data['lines'] = data['lines'][:4]
            print('[Dimoi French] Generated dialogue: ' + str(data.get('topic')))
            return data
    except Exception as e:
        print('[Dimoi French] Fallback triggered: ' + str(e))

    return {
        'topic': category,
        'lines': [
            {'speaker': 'A', 'english': 'Hello! Is this croissant fresh?', 'target_text': 'Bonjour! Ce croissant est frais?', 'japanese': 'Bonjour! Ce croissant est frais?', 'romaji': ''},
            {'speaker': 'B', 'english': 'Yes, it is warm and fresh!', 'target_text': 'Oui, il est bien chaud!', 'japanese': 'Oui, il est bien chaud!', 'romaji': ''},
            {'speaker': 'A', 'english': 'One, please.', 'target_text': 'Un, s’il vous plaît.', 'japanese': 'Un, s’il vous plaît.', 'romaji': ''},
            {'speaker': 'B', 'english': 'Here you go, enjoy!', 'target_text': 'Voilà, bon appétit!', 'japanese': 'Voilà, bon appétit!', 'romaji': ''}
        ]
    }
...
def render_frame(speaker_speaking, is_talking, mouth_flap, line_data, font_en, font_jp, font_ro):
    ...
    raw_french = line_data.get('target_text', line_data.get('japanese', ''))
    wrapped_jp = wrap_text(raw_french, font_jp, max_w=920)
    ...
    gap_1 = 55  # Gap between English and French
...
async def generate_single_reel():
    ...
    print('\n[Dimoi French] Generated: ' + str(final_vid.name))
    return final_vid
